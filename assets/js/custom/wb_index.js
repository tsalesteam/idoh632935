...
function openGenerateSeoPublishModal(publishFunction) {
    ...
    $("#publishToServerBtn")
        .prop("disabled", false)
        .html("Publish to Server");

    if (typeof prefillSeoPublishForm === "function") {
...
function getSeoPublishBusinessData() {
    ...
    return {
        ...
        hours: {
            Monday: value("seoMonday"),
            Tuesday: value("seoTuesday"),
            Wednesday: value("seoWednesday"),
            Thursday: value("seoThursday"),
            Friday: value("seoFriday"),
...
async function updateSEO() {
    ...
    if (!clientDomain) {
        throw new Error("Please enter the client domain.");
    }
    ...
    console.log("SEO client domain:", clientDomain);
    console.log("SEO client name:", clientName);
    console.log("SEO project name:", projectName);
    console.log("SEO business data:", JSON.stringify(businessData, null, 4));
...
function prefillSeoPublishForm() {
    ...
    const clientDomain = getCookie("clientDomain") || "";

    $("#seoClientDomain").val(clientDomain);
    ...
    if (!$("#seoMonday").val()) {
        $("#seoMonday").val("09:00-18:00");
    }

    if (!$("#seoTuesday").val()) {
        $("#seoTuesday").val("09:00-18:00");
    }

    if (!$("#seoWednesday").val()) {
        $("#seoWednesday").val("09:00-18:00");
    }

    if (!$("#seoThursday").val()) {
        $("#seoThursday").val("09:00-18:00");
    }

    if (!$("#seoFriday").val()) {
        $("#seoFriday").val("09:00-18:00");
    }
...
$(document).on("click", "#publishToServerBtn", async function () {
    ...
    try {
        ...
        btn.html("Publish to Server");
    }

});

$(document).on("blur", "#seoClientDomain", function () {

    const domainInput = $(this);

    const domain =
        domainInput.val().trim().toLowerCase();

    const projectName =
        (getCookie("projectName") || "").trim().toLowerCase();

    const errorBox =
        $("#seoClientDomainError");
    ...
    const isValid =
        projectParts.some(part =>
            domainText.includes(part)
        );
    ...
    if (!isValid) {
        ...
        errorBox
            .text(
                "Domain should include part of the project name: " +
                originalProjectName
            )
            .show();
